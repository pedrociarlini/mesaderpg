...
public class ConectarJogadorAction extends AbstractAction {
	...
	public ConectarJogadorAction() {
		...
		putValue(Action.LONG_DESCRIPTION,
				"Conecta um jogador que esteja na rede.");
	}
		...
		try {
			...
			if (conn != null) {
				...
				if (!JogadoresBusiness.CONEXAO_START.equals(msg)) {
					throw new Exception(
							"Erro na conexão: Sequência de conexão não esperada.");
				}
				conn.send(jogador);
				...
				conn.addDataReceivedListener(JogadoresBusiness
						.createJogadorListener(remoteJogador.getNome()));
				JogadoresBusiness.conectarJogador(remoteJogador);
				listaJogadores
						.setListData(getJogadorComponents(JogadoresBusiness
								.getListaJogadores()));
			}
		} catch (ClassCastException ex) {
			MensagensUtil
					.showMensagemErro("Erro durante o handshake com o jogador remoto.");
		} catch (Exception ex) {
